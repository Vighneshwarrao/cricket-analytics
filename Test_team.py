import pandas as pd
Score=pd.read_csv('Test_Scores.csv')


players=pd.read_csv('Cleaned_datasets/Players.csv')

team=players[['Player ID','Player Name','Test Batting Pos','Playing Role','Batting Style','Bowling Style','Bowling Type']].merge(
...
if(num_bowlers+num_openers+num_top_order+num_allrounders+num_middle_order+num_keeper==11):
    ...
    #Openers Selection
    openers=team[team['Test Batting Pos']=='Openers'].sort_values('Batsmen Score',ascending=False).head(num_openers)['Player Name'].reset_index(drop=True)

    #Top Order Selection
    top_order=team[team['Test Batting Pos']=='Top Order'].sort_values('Batsmen Score',ascending=False).head(num_top_order)['Player Name'].reset_index(drop=True)

    #Middle Order Selection
    middle_order=team[team['Test Batting Pos']=='Middle Order'].sort_values('Batsmen Score',ascending=False).head(num_middle_order)['Player Name'].reset_index(drop=True)

    #Keeper Selection
    Keeper=team[(team['Test Batting Pos']=='Middle Order') & (team['Playing Role']=='Wicketkeeper Batter')].sort_values('Keeper Score',ascending=False).head(num_keeper)['Player Name'].reset_index(drop=True)

    #Allrounder Selection
    all_pace=team[((team['Playing Role']=='Allrounder') | (team['Playing Role']=='Bowling Allrounder') | (team['Playing Role']=='Batting Allrounder')) & (team['Bowling Type'].str.contains('Pacer'))].sort_values(['Allrounder Score','Batsmen Score'],ascending=False).head(num_allrounders).reset_index(drop=True)
    all_spin=team[((team['Playing Role']=='Allrounder') | (team['Playing Role']=='Bowling Allrounder') | (team['Playing Role']=='Batting Allrounder')) & (team['Bowling Type']=='Spin')].sort_values(['Allrounder Score','Batsmen Score'],ascending=False).head(num_allrounders).reset_index(drop=True)
    ...
    #Pacer Selection
    pacers=team[(team['Playing Role']=='Bowler') &(team['Bowling Type']).str.contains('Pacer') ].sort_values('Bowler Score',ascending=False).head(rem_pacers).reset_index(drop=True)

    #Spinner Selection
    spinners=team[(team['Playing Role']=='Bowler') &(team['Bowling Type']=='Spin')].sort_values('Bowler Score',ascending=False).head(rem_spinners).reset_index(drop=True)
